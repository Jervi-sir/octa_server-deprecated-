...
class OpCollectionController extends Controller
{
    ...
    public function removeStoreFromCollection(Request $request)
    {
        ...
        $existsInOtherCollections = Collection::where('user_id', $userId)
                                            ...
                                            ->whereHas('rls_shops', function($query) use ($shopId) {
    ...
    public function getCollectionDetails(Request $request)
    {
        ...
        $collection = Collection::where('id', $collectionId)
            ...
            ->with('rls_shops') // Assuming 'stores' is the relationship name
            ->first();
    ...
    public function updateCollection(Request $request)
    {
        $request->validate([
            ...
            'thumbnail_base64' => 'nullable|string|max:2048', // Assuming thumbnail is an image file
        ]);
        ...
        if ($request->has('thumbnail_base64')) {
            ...
            Storage::put($imagePath, base64_decode($request->thumbnail_base64));
            $collection->thumbnail = Storage::url($imagePath);
        }
